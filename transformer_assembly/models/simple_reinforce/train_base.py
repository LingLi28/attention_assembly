...
if __name__ == "__main__":
    ...
    # Initialize the BaseStockPolicy
        ...
        save_model_cat = "simple_RL"
        
        save_path = f"./transformer_assembly/pretrained/{save_model_cat}/base_RL/RL_{config_name}_ep_{num_episodes}.pth"
        
        fig_save_path = f"./transformer_assembly/img/{save_model_cat}/base_RL/loss_{config_name}_ep_{num_episodes}.png"

        
        reinforce_agent = REINFORCE(attention_env, base_env, base_greedy_env, base_policy, lr=1e-4)

        reinforce_agent.train(num_episodes=num_episodes, max_time_steps=max_time_steps, save_path=save_path)
        ...
        reinforce_agent.evaluate(num_episodes=1000, max_time_steps=100, use_baseline=False, save_results=True, result_prefix="base_RL")
